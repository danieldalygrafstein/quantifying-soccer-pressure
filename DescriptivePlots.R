...
library(tidyverse)
library(reshape2)
source("Rcode/soccerPitch.R")


statsbomb_events <- read.csv("events/Statsbombevents_xTadded.csv", row.names = 1)
...
statsbomb_events_trimmed$location.third <- apply(statsbomb_events_trimmed, 1, function(x)  if (x[33]<25){"defending"}
                                                                                          else if (x[33]<73){"midfield"}
                                                                                          else {"attacking"} )
...
turnovers_pct_location <- turnovers_pct_location[c(1:6,22:24),]
